...
import com.seeapenny.client.OrderInfo;
import com.seeapenny.client.PurchasedLogic;
...
//         if (Consts.DEBUG) Log.i(TAG, getClass().getSimpleName());
         
         if (mService != null) {
            try {
               mRequestId = run();
//               if (Consts.DEBUG) Log.i(TAG, "request id: " + mRequestId);
...
//         if (Consts.DEBUG) Log.i(TAG, "remote billing service crashed");
         mService = null;
      }
...
//         if (Consts.DEBUG) Log.i(TAG, method + " received " + responseCode.toString());
      }
   }
...
//         if (Consts.DEBUG) Log.i(TAG, "CheckBillingSupported response code: " + ResponseCode.valueOf(responseCode));
         
         boolean billingSupported = (responseCode == ResponseCode.RESULT_OK.ordinal());
         purchaseObserver.onBillingSupported(billingSupported);
...
//            if (Consts.DEBUG) Log.i(TAG, "Error with requestPurchase");
            return Consts.BILLING_RESPONSE_INVALID_REQUEST_ID;
         }
      ...
      @Override
      protected void responseCodeReceived(ResponseCode responseCode) {
         purchaseObserver.onRequestPurchaseResponse(this, responseCode);
      }
   }
   ...
   public class RestoreTransactions extends BillingRequest {
      ...
      @Override
      protected void responseCodeReceived(ResponseCode responseCode) {
         purchaseObserver.onRestoreTransactionsResponse(this, responseCode);
      }
   }
   ...
   public static void setPurchaseObserver(PurchaseObserver purchaseObserver) {
      BillingService.purchaseObserver = purchaseObserver;
   }
   ...
   /**
    * The {@link BillingReceiver} sends messages to this service using intents. Each intent has an action and some extra
    * arguments specific to that action.
    * 
    * @param intent
...
//      if (Consts.DEBUG) BillingReceiver.debugIntent(TAG, "handleCommand", intent);
...
//         if (Consts.DEBUG) Log.i(TAG, "binding to Market billing service");
         ...
         boolean bindResult = bindService(new Intent(Consts.MARKET_BILLING_SERVICE_ACTION), this,
    ...
    * {@link ResponseHandler#purchaseResponse(Context, PurchaseState, String, String, long)} for each verified purchase.
    * 
    * @param startId
    *           an identifier for the invocation instance of this service
    * @param signedData
   ...
   private void purchaseStateChanged(int startId, String signedData, String signature) {
      // Toast.makeText(Launcher.getInstance(), "purchaseStateChanged",
      ...
      for (VerifiedPurchase vp : purchases) {
         ...
         purchaseResponse(vp.purchaseState, vp.productId, vp.orderId, vp.purchaseTime, vp.developerPayload);
      }
   ...
   private void purchaseResponse(PurchaseState purchaseState, String productId, String orderId, long purchaseTime,
         String developerPayload) {
      // Toast.makeText(MeetApp.getInstance(), "purchaseResponse: " + productId
      // + " state: " + purchaseState, Toast.LENGTH_SHORT).show();
      // if (client.isConnected()) {
      if (purchaseState == PurchaseState.PURCHASED) {
         OrderInfo orderInfo = new OrderInfo(orderId, productId, developerPayload);
         SeeapennyApp.getInstance().savePurchase(orderInfo);
   ...
   private void checkResponseCode(long requestId, ResponseCode responseCode) {
      ...
      if (request != null) {
//         if (Consts.DEBUG) Log.i(TAG, request.getClass().getSimpleName() + ": " + responseCode);
...
//      if (Consts.DEBUG) Log.i(TAG, "Billing service connected");
...
//      if (Consts.DEBUG) Log.i(TAG, "Billing service disconnected");
      mService = null;
   }
