...
//      if (Consts.DEBUG) debugIntent(TAG, "onReceive", intent);
...
//         if (Consts.DEBUG) Log.i(TAG, "notifyId: " + notifyId);
         ...
         // }
         // else if (Consts.ACTION_INSTALL_REFERRER.equals(action)) {
         // Bundle extras = intent.getExtras();
         // String referrer = extras.getString("referrer");
         //
         // Log.d("BillingReceiver", "referrer: " + referrer);
         //
         // MeetApp app = MeetApp.getInstance();
         // if (referrer != null) {
         // app.saveMarketReferrer(referrer);
         // }
         // app.setRecentlyInstalled(true);
         // app.registerC2DM();
         //
         // MdotmReceiver mdotm = new MdotmReceiver();
         // mdotm.onReceive(context, intent);
         //
         // AppiaReceiver appia = new AppiaReceiver();
         // appia.onReceive(context, intent);
         
         // forwardToOtherReceivers(context, intent);
         
      } else {
//         if (Consts.DEBUG) Log.i(TAG, "unexpected action: " + action);
      }
   }
   
   // private void forwardToOtherReceivers(Context context, Intent intent) {
   // ComponentName name = new ComponentName(context, BillingReceiver.class);
   // ActivityInfo ai;
   // try {
   // ai = context.getPackageManager().getReceiverInfo(name,
   // PackageManager.GET_META_DATA);
   // } catch (NameNotFoundException nnfex) {
   // if (Consts.DEBUG) Log.e(TAG, "forwardToOtherReceivers", nnfex);
   // return;
   // }
   //
   ...
   // } catch (Exception ex) {
   // if (Consts.DEBUG) Log.e(TAG, "forwardToOtherReceivers", ex);
   // }
   // if (obj instanceof BroadcastReceiver) {
   // BroadcastReceiver receiver = (BroadcastReceiver) obj;
   ...
   public static void debugIntent(String tag, String prefix, Intent intent) {
      Log.i(tag, prefix + ": " + intent);
      Log.i(tag, prefix + " action: " + intent.getAction());
      Log.i(tag, prefix +" extras begin:");
      Bundle extras = intent.getExtras();
      if (extras != null) {
