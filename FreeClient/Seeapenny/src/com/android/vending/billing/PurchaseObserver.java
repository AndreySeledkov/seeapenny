...
public class PurchaseObserver {
   ...
   private static final Class[] START_INTENT_SENDER_SIG = new Class[] { IntentSender.class, Intent.class, int.class,
         int.class, int.class };
   ...
   public void onBillingSupported(boolean supported) {
      // Toast.makeText(Launcher.getInstance(), "onBillingSupported",
   ...
   public void onPurchaseStateChange(PurchaseState purchaseState, String itemId, int quantity, long purchaseTime,
      ...
      // Toast.makeText(Launcher.getInstance(), "onPurchaseStateChange: " +
      // purchaseState, Toast.LENGTH_LONG).show();
   }
   
   /**
    * This is called when we receive a response code from Market for a RequestPurchase request that we made. This is NOT
    * used for any purchase state changes. All purchase state changes are received in
    * {@link #onPurchaseStateChange(PurchaseState, String, int, long)}. This is used for reporting various errors, or if
    * the user backed out and didn't purchase the item. The possible response codes are: RESULT_OK means that the order
    * was sent successfully to the server. The onPurchaseStateChange() will be invoked later (with a purchase state of
    * PURCHASED or CANCELED) when the order is charged or canceled. This response code can also happen if an order for a
    * Market-managed item was already sent to the server. RESULT_USER_CANCELED means that the user didn't buy the item.
    * RESULT_SERVICE_UNAVAILABLE means that we couldn't connect to the Android Market server (for example if the data
    * connection is down). RESULT_BILLING_UNAVAILABLE means that in-app billing is not supported yet.
   ...
   public void onRequestPurchaseResponse(RequestPurchase request, ResponseCode responseCode) {
      // String href = Client.PR_PREF_CHECKOUT + request.mProductId + "\\" +
      // request.mDeveloperPayload;
      // Toast.makeText(Launcher.getInstance(), responseCode +
      // " onRequestPurchaseResponse: " + href, Toast.LENGTH_SHORT).show();
   }
   
   /**
    * This is called when we receive a response code from Android Market for a RestoreTransactions request that we made.
    * A response code of RESULT_OK means that the request was successfully sent to the server.
    */
   public void onRestoreTransactionsResponse(RestoreTransactions request, ResponseCode responseCode) {
      // Toast.makeText(Launcher.getInstance(),
   ...
   void startBuyPageActivity(PendingIntent pendingIntent, Intent intent) {
      if (mStartIntentSender != null) {
         ...
         // This is on Android version 1.6. The in-app buy page activity must be
         // on its
