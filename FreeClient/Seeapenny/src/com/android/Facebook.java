...
public class Facebook {
    ...
    * 
    * IMPORTANT: Note that single sign-on authentication will not function correctly if you do not include a call to the
    * authorizeCallback() method in your onActivityResult() function! Please see below for more information. single
    ...
    *           "offline_access", etc. see http://developers.facebook.com/docs/authentication/permissions This parameter
    *           should not be null -- if you do not require any permissions, then pass in an empty String array.
    * @param activityCode
    *           Single sign-on requires an activity result to be called back to the client application -- if you are
    *           waiting on other activities to return data, pass a custom activity code here to avoid collisions. If you
    *           would like to force the use of legacy dialog-based authorization, pass FORCE_DIALOG_AUTH for this
    ...
    * 
    * If your calling activity does not currently implement onActivityResult(), you must implement it and include a call
    * to this method if you intend to use the authorize() method in this SDK.
    * 
      ...
      if (requestCode == mAuthActivityCode) {
         ...
         if (resultCode == Activity.RESULT_OK) {
            ...
            if (data != null) {
               ...
               mAuthDialogListener.onError(new DialogError(data.getStringExtra("error"), data.getIntExtra("error_code",
                     -1), data.getStringExtra("failing_url")));
    ...
    *           Callback interface for notifying the calling application when the refresh request has completed or
    *           failed (can be null). In case of a success a new token can be found inside the result Bundle under
    ...
    *
    * @param parameters
    *           Key-value pairs of parameters to the request. Refer to the documentation: one of the parameters must be
    *           "method".
    * @throws java.io.IOException
   ...
   /**
    * Synchronously make a request to the Facebook Graph API with the given HTTP method and string parameters. Note that
    * binary data parameters (e.g. pictures) are not yet supported by this helper function.
    *
    * See http://developers.facebook.com/docs/api
    *
    * Note that this method blocks waiting for a network response, so do not call it in a UI thread.
